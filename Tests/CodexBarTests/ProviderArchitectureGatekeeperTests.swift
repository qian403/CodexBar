...
struct ProviderArchitectureGatekeeperTests {
    ...
    func `descriptor widget colors preserve the pre-derivation literals`() {
        ...
        #expect(widgetFingerprint == 269_659_858_153_560_751)
        #expect(burnDownFingerprint == 17_432_916_017_947_454_013)
    }
    ...
    func `small provider capabilities preserve legacy registries`() {
        ...
        #expect(Set(descriptors.filter(\.tokenCost.supportsTokenSnapshot).map(\.id)) == [
            .codex, .claude, .cursor, .vertexai, .bedrock, .opencode, .opencodego,
        ])
        #else
        #expect(Set(descriptors.filter(\.tokenCost.supportsTokenSnapshot).map(\.id)) == [
    ...
    func `cross provider case clusters are derived or specifically justified`() throws {
        ...
        // Custom slots are one configurable feature family, and the `custom` ID collides with ubiquitous
        // non-provider `.custom` enum cases. Manifest and registry tests cover their generated registrations.
        let providerIDs = Set(UsageProvider.allCases.map(\.rawValue))
            .subtracting(UsageProvider.customSlots.map(\.rawValue))
    ...
    private static let allowedProviderConstructs: [AllowedProviderConstruct] = [
        ...
        AllowedProviderConstruct(
            path: "Sources/CodexBarCore/CostUsageFetcher.swift",
            line: 616,
            anchor: "options.provider == .codex || options.provider == .claude || options.provider == .opencode",
            expectedProviderIDs: ["claude", "codex", "opencode", "opencodego"],
            expectedReferenceCount: 4,
            expectedReferenceFingerprint: ["claude@0", "codex@0", "opencode@0", "opencodego@1"],
            reason: "This exact cost scanner dispatch selects a provider-owned transcript, cache, or pricing format."),
        AllowedProviderConstruct(
            path: "Sources/CodexBarCore/CostUsageFetcher.swift",
            line: 644,
        ...
        AllowedProviderConstruct(
            ...
            line: 1435,
            anchor: "if provider == .cursor {",
            expectedProviderIDs: ["bedrock", "cursor", "opencode", "opencodego"],
            expectedReferenceCount: 4,
            expectedReferenceFingerprint: ["bedrock@0", "opencode@12", "opencodego@12", "cursor@24"],
            reason: "This exact cost scanner dispatch selects a provider-owned transcript, cache, or pricing format."),
        AllowedProviderConstruct(
            path: "Sources/CodexBarCore/LocalAgentSessionScanner.swift",
            line: 93,
