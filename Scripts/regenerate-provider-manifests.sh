...
for provider in "${PROVIDERS[@]}"; do
  if [[ "$provider" == custom* ]]; then
    DESCRIPTOR_TYPES+=("CustomProviderDescriptors.descriptor(for: .${provider})")
    IMPLEMENTATION_TYPES+=("CustomProviderImplementation(id: .${provider})")
    continue
  fi
  ...
  DESCRIPTOR_TYPES+=("${descriptor_type}.descriptor")
